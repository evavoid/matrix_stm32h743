...
void atk8266_sta_init(u8 mode,u8 ipbuf[16])//UDP0 TCPS1 TCPC2
{
	...
	/*****STA主体配置******/
	printf("开始配置STA\r\n");
	atk_8266_send_cmd("AT+CWMODE=1","OK",100);//设置为STA模式
	atk_8266_send_cmd("AT+RST","OK",100);//重启
	delay_ms(1000);	delay_ms(1000);	delay_ms(1000); delay_ms(1000);//重启等待
	//设置连接到的WIFI网络名称/加密方式/密码,这几个参数需要根据您自己的路由器设置进行修改!! 
	sprintf((char*)sta_string,"AT+CWJAP=\"%s\",\"%s\"",wifista_ssid,wifista_password);//设置无线参数:ssid,密码
	while(atk_8266_send_cmd(sta_string,"WIFI GOT IP",300));					//连接目标路由器,并且获得IP
	/*****STA细节配置******/
	if(mode==0)//UDP
	{
		...
		sprintf((char*)sta_string,"AT+CIPSTART=\"UDP\",\"%s\",%s",ipbuf,(u8*)portnum);    //配置目标UDP服务器
		while(atk_8266_send_cmd(sta_string,"OK",500));//反复连接目标机
	...
	else if(mode==1)//TCP Client
	{
		...
		atk_8266_send_cmd("AT+CIPMUX=0","OK",100);  //单链接模式
		sprintf((char*)sta_string,"AT+CIPSTART=\"TCP\",\"%s\",%s",ipbuf,(u8*)portnum);    //配置目标UDP服务器
		while(atk_8266_send_cmd(sta_string,"OK",200));//反复连接目标机	
	...
	else if(mode==2)//TCP Server
	{
		...
		sprintf((char*)sta_string,"AT+CIPSERVER=1,%s",(u8*)portnum);
		atk_8266_send_cmd(sta_string,"OK",20);//配置TCP主机
		printf("TCP_Server配置成功\r\n");
		atk_8266_get_wanip(ipbuf);//服务器模式,获取IP
		sprintf((char*)sta_string,"IP地址:%s 端口:%s",ipbuf,(u8*)portnum);printf("%s\r\n",sta_string);
		/****等待连接******/
		printf("请用设备连接WIFI热点:%s,%s,%s",(u8*)wifiap_ssid,(u8*)wifiap_encryption,(u8*)wifiap_password);
	}
}
void atk8266_sta_data_transform(u8 mode,u8 stop,u32 byte_size,u8* data)
{
	u8 tem_string[32];
	if(stop)
	{
