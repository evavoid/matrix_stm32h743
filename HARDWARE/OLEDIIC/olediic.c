...
{ 	
  ...
  HAL_GPIO_WritePin(GPIOH, GPIO_PIN_12|GPIO_PIN_13, GPIO_PIN_SET);
}  
...
{
	if(OLED_state->ready_state==prepare_state)
	{
		OLED_Init();
		OLED_state->ready_state=run_state0;
	}
	else if(OLED_state->ready_state==run_state0)
	{
	}
}
