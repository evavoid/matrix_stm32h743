...
u32 LCD_ReadPoint(u16 x,u16 y)
{
	...
	if(x>=lcddev.width||y>=lcddev.height)return 0;	//超过了范围,直接返回
 	LCD_SetCursor(x,y);	    
	if(lcddev.id==0X5510)LCD_WR_REG(0X2E00);	//5510 发送读GRAM指令
...
/**********************************************************************************/
//网络
void LCD_Network(u16 x1,u16 y1,u16 x2,u16 y2,u16 xwidth,u16 ywidth)//xwidth???????
{
	
	//LCD_DrawLine(u16 x1, u16 y1, u16 x2, u16 y2);
	u8 xlines=abs(x1-x2)/xwidth+1;//?????
	u8 ylines=abs(y1-y2)/ywidth+1;
	u8 i=0;
	POINT_COLOR=RED;
	for (i=0;i<xlines;i++)LCD_DrawLine(x1+i*xwidth,LCD_y(y1),x1+i*xwidth,LCD_y(y2));
	for (i=0;i<ylines;i++)LCD_DrawLine(x1,LCD_y(y1+i*ywidth),x2,LCD_y(y1+i*ywidth));
}
//翻转
u16 LCD_y(u16 y)
{
	return SSD_VER_RESOLUTION-y;
}
//x轴
void LCD_xaxisw(u16 yoffset,u16 x0,u16 x1,u8 width)
{
	POINT_COLOR=RED;
	LCD_Fill(x0,LCD_y(yoffset+width/2),x1,LCD_y(yoffset-width/2),POINT_COLOR);
}
//y轴
void LCD_yaxisw(u16 xoffset,u16 y0,u16 y1,u8 width)
{
	POINT_COLOR=RED;
	LCD_Fill(xoffset-width/2,LCD_y(y1),xoffset+width/2,LCD_y(y0),POINT_COLOR);	
}
//波窗口
void LCD_wave_show(u16 x1,u16 y1,u16 x2,u16 y2,float xrange0,
	float xrange1,float yrange0,float yrange1,float *posy,u16 data_num)//?????????xrange0????x????
{
	float xgain=(float)(x2-x1)/(float)(xrange1-xrange0);
	float ygain=(float)(y2-y1)/(float)(yrange1-yrange0);
	u32 i=0;
	POINT_COLOR=YELLOW;
	for(i=0;i<data_num-1;i++)
	{
		if(posy[i]>=yrange0 && yrange1>=posy[i] && i>=xrange0 && xrange1>=i)//前置点检测//横向省略检测
		{
			if(posy[i+1]<yrange0 || yrange1<posy[i+1])// || i+1<xrange0 || xrange1<i+1)//后置点检测
				posy[i+1]=posy[i];
			LCD_DrawLine
			( 
				(int)(x1+(float)(i-xrange0)*xgain),
				(int)LCD_y(y1+(float)(posy[i]-yrange0)*ygain),
				(int)(x1+(float)(i+1-xrange0)*xgain),
				(int)LCD_y(y1+(float)(posy[i+1]-yrange0)*ygain)
			);
		}
	}
	return;
...
{ 
	LCD_xaxisw(ypos,xpos-5,xpos+5,2);
	LCD_yaxisw(xpos,ypos-5,ypos+5,2);
}

void LCD_erase(u16 x1,u16 y1,u16 x2,u16 y2)
{
	LCD_Fill(x1,LCD_y(y2),x2,LCD_y(y1),BLACK);
}
void hist_show(float * data,u16 blocksize,u16* output,u16 outputsize)
{
	float min,max;
	u32 step;//区间长度
	u32 index;
	
	arm_max_f32(data,blocksize,&max,&index);
	arm_min_f32(data,blocksize,&min,&index);
	step=(u32)((max-min+2)/outputsize);
	memset(output,0,sizeof(output[0]));
	for(index=0;index<blocksize;index++)
	{
		output[( (u32)(data[index]-min) ) /step]++;
	}
} 
/**********************************************************************************/
void lcd_event(local_state_space *LCD_state)//每一个底层判断里都跟一个return,比较保险
{
	if(LCD_state->ready_state==prepare_state)
	{
		LCD_Init();
		LCD_state->ready_state=run_state0;
	}
	else if(LCD_state->ready_state==run_state0)
	{
		LCD_Clear(BLACK);
		LCD_xaxisw(240,40,760,4);
		LCD_yaxisw(400,20,460,4);
		LCD_Network(1,1,800,480,100,60); 
		//LCD_wave_show(20,40,700,450,0,750,0,4,float_data,global_data.float_data,750);
	}
}
